...
void printer::print_states(const char *filename, std::vector<Eigen::Vector4d> &states)
{
...
void printer::print_states(const char *filename, dmod::array1d &background, dmod::array1d &amplitude, dmod::array1d &frequency, dmod::array1d &phase)
{
...
void printer::print_signal(const char *filename, dmod::array1d &signal)
{
...
void printer::print_full_Kalman_states(const char *filename, std::vector<ExtendedKalmanFilterIS1DState> &states)
{
	size_t N = states.size();
	std::ofstream out(filename);
	for (int i = 0; i < N; i++)
...
void printer::console_print_full_Kalman_state(ExtendedKalmanFilterIS1DState &state)
{
...
void printer::print_Kalman_stdev(const char *filename, std::vector<Eigen::Vector4d> &states, dmod::array1d &signal, dmod::array1d &noise, 
...
void printer::console_print_Kalman_stdev(std::vector<Eigen::Vector4d> &states, dmod::array1d &signal, dmod::array1d &noise,
	dmod::array1d &background, dmod::array1d &amplitude, dmod::array1d &frequency, dmod::array1d &phase, dmod::array1d &restoredSignal)
{
	size_t N = states.size();
	if (signal.size() != N || noise.size() != N || background.size() != N || amplitude.size() != N || frequency.size() != N || phase.size() != N || restoredSignal.size() != N)
