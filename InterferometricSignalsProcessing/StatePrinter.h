...
namespace StatePrinter
{
	...
	void print_Kalman_stdev(char *filename, Eigen::Vector4d *states, double *background, double *amplitude, double *frequency, double *phase, int N);

	//console
	void console_print_full_Kalman_state(ExtendedKalmanFilterIS1DState &state);
	void console_print_Kalman_stdev(char *filename, Eigen::Vector4d *states, double *background, double *amplitude, double *frequency, double *phase, int N);
}
